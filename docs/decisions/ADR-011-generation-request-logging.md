...
요청당 로그 수와 단계 요약 원칙은 유지한다. 로그에 노출하는 필드와 upstream 로그 정제
계약은 ADR-025가 개정한다.
...
- `[request]`는 TAESD·LoRA 상태와 최종 성공 여부·전체 시간을 기록합니다. 모델·출력·
  양자화 경로와 파일명은 기록하지 않습니다.
- `[settings]`는 해상도, preset과 실제 scheduler, steps, CFG, Hires 방식·배율·steps·
  denoising strength를 기록합니다. prompt 본문과 길이, seed, 출력 경로는 기록하지
  않습니다.
...
- `[stage]`는 loading, encoding, sampling, decoding, PNG write의 wall time만 초 단위로
  기록합니다.
- upstream 콜백의 info/debug 로그는 일반 logcat에 출력하지 않고, warning/error만
  정제해 전달합니다. 단, 크래시 보고서의 `nativeTail`에는 마지막 성공 지점과 실행 순서를
  보존하기 위해 정제된 모든 레벨의 최근 로그를 최대 40줄 보관합니다. 콜백의 info 문자열은
  단계 경계 판별과 UI progress event에도 계속 사용합니다.
...
- 모델별 크래시 분석에는 직접 식별자 대신 family, component storage와 추정 byte 같은
  기술 특성을 사용합니다.
